...
void createText(vsg::CommandLine& arguments, vsg::Window* window)
{
  ...
  //  auto font_filename = arguments.value(std::string("I:\\vsg\\vsgExamples\\data\\fonts\\times.vsgb"), "-f");
    ...
    layout->position = vsg::vec3(10.0f, window->extent2D().height - 60.0f, 0.5f);
...
int main(int argc, char** argv)
{
   ...
   /*      auto commandGraph = vsg::createCommandGraphForView(window, camera, sceneGraph);
        ...
        while (viewer->advanceToNextFrame())
        {
            ...
            double t = std::chrono::duration<double, std::chrono::seconds::period>(now - lastTime).count() * 1000.0;

            if (t > 300)
            {
                double fps = frameCount / t;
                std::cout << "FPS: " << 1000 / fps << std::endl;
                fps_text->text = vsg::stringValue::create("FPS: " + std::to_string(static_cast<int>(1000.0 / fps)));
